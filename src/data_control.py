...
class Data_Control():
    ...
    def LT_trand_check(self, df):
        ...
        def check_ma_trend(sma20, sma60, sma120, rbw, tol_ratio=0.005):
            ...
            # -------------------------------------------------------
            # (A) 20일과 60일 이평만 근접, 120일과는 차이
            # -------------------------------------------------------
            if near_20_60 and not near_20_120 and not near_60_120:
                # 20,60이 같은 선상 -> '단기 박스권' 형성
                # -> 나머지 120과의 상대적 위치가 어떠한지에 따라 세분화
                merged_20_60 = (sma20 + sma60) / 2

                if merged_20_60 < sma120:
                    return 11  # Code 11: (20≈60) < 120
                              # 단기 박스권이 장기 이평 아래에 -> '상승 시도' or '상승 초입' 가능

                elif merged_20_60 > sma120:
                    return 12  # Code 12: (20≈60) > 120
                              # 단기 박스권이 장기 이평 위에 -> '약한 하락' or '조정' 시나리오

            # -------------------------------------------------------
            # (B) 20일과 120일 이평만 근접, 60일과는 차이
            # -------------------------------------------------------
            if near_20_120 and not near_20_60 and not near_60_120:
                # 20,120이 같은 선상 -> 단기와 장기의 흐름이 비슷
                # -> 60과 비교
                merged_20_120 = (sma20 + sma120) / 2

                if merged_20_120 < sma60:
                    return 13  # Code 13: (20≈120) < 60
                              # 중장기가 60일보다 낮은 위치 -> 단기·장기가 상대적으로 약세, 
                              # 중기(60)만 더 높은 이례적 상황

                elif merged_20_120 > sma60:
                    return 14  # Code 14: (20≈120) > 60
                              # 중장기가 60일보다 높은 위치 -> 단기+장기가 중간보다 강세,
                              # 60만 낮은 이례적 상황

            # -------------------------------------------------------
            # (C) 60일과 120일 이평만 근접, 20일과는 차이
            # -------------------------------------------------------
            if near_60_120 and not near_20_60 and not near_20_120:
                # 60,120이 같은 선상 -> 중기와 장기 흐름이 유사
                # -> 20과 비교해 단기 편차가 크다는 의미
                merged_60_120 = (sma60 + sma120) / 2

                if merged_60_120 < sma20:
                    return 15  # Code 15: (60≈120) < 20
                              # 중장기가 낮고, 단기가 위 -> '최근 단기 급등' or '단기 상승 모멘텀'

                elif merged_60_120 > sma20:
                    return 16  # Code 16: (60≈120) > 20
                              # 중장기가 더 높은 위치, 단기가 더 아래 -> '단기 약세'
